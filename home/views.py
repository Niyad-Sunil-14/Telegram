        ...
        if last_message:
            if last_message.image:
                if last_message.body:
                    ...
                    last_message.body = "You sent a photo" if last_message.author == user else "Sent you a photo"
...
@login_required
def upload_chat_media(request, chatroom_name):
    if request.method == "POST":
        chat_group = get_object_or_404(ChatGroup, group_name=chatroom_name)
        form = ChatmessageCreateForm(request.POST, request.FILES)
        if form.is_valid():
            ...
            if 'audio' in request.FILES:
                ...
                if audio_file.size > 5 * 1024 * 1024:
                    return JsonResponse({'success': False, 'error': 'Audio file too large'})
                ...
                try:
                    ...
                    with open(temp_output_path, 'rb') as processed_file:
                        message.audio.save(audio_file.name.replace('.webm', '.mp3'), processed_file, save=False)
                finally:
